...
function init() {
  ...
  document.getElementById('popupBtn').addEventListener('click', openInPopup);
...
function applySettings() {
  chrome.storage.local.get(['settings'], (result) => {
    if (result.settings) {
      ...
      // 设置弹出窗口最小大小，而不是固定大小，以允许调整
      if (settings.popupWidth && settings.popupHeight) {
        document.body.style.minWidth = `${settings.popupWidth}px`;
...
function openInPopup() {
  ...
  let width = Math.floor(screenWidth * 2 / 3);
  let height = Math.floor(screenHeight * 2 / 3);
  
  // 从存储中获取设置的宽度和高度
  chrome.storage.local.get(['settings'], (result) => {
    if (result.settings && result.settings.popupWidth && result.settings.popupHeight) {
      width = result.settings.popupWidth;
      height = result.settings.popupHeight;
    }
    
    // 创建新的弹出窗口
    chrome.windows.create({
      url: chrome.runtime.getURL('imagepage.html'),
      type: 'popup',
      width: width,
      height: height
    });
...
function detectImages() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
    if (tabs.length > 0) {
      ...
      console.log('检测标签页:', tabId);
      
      chrome.tabs.sendMessage(tabs[0].id, {action: 'detectImages'}, (response) => {
        ...
        if (response && response.images) {
          console.log('检测到图片:', response.images.length);
          images = response.images;
          displayImages(images);
          setStatus(`检测到 ${images.length} 张图片`);
          
          // 存储图片数据到chrome.storage
          const tabTitle = tabs[0].title;
          chrome.storage.local.set({
            [`images_${tabId}`]: images,
            'lastDetectedTab': tabId,
          ...
          displayImages([]);
          // 清除存储的图片数据
          chrome.storage.local.set({[`images_${tabId}`]: []});
        }
      });
    } else {
      console.log('未找到标签页');
...
function applyFilter() {
  ...
  chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
    if (tabs.length > 0) {
      currentTabId = tabs[0].id;
      chrome.tabs.sendMessage(tabs[0].id, {action: 'filterImages', filters: filters}, (response) => {
        if (chrome.runtime.lastError) {
          console.error('发送消息失败:', chrome.runtime.lastError);
        ...
        if (response && response.images) {
          ...
          const tabId = tabs[0].id;
          const tabTitle = tabs[0].title;
          chrome.storage.local.set({[`images_${tabId}`]: images, lastDetectedTab: tabId, lastDetectedTabTitle: tabTitle});
        } else {
          setStatus('过滤后无图片');
          displayImages([]);
        }
...
function displayImages(imageList) {
  ...
  imageList.forEach((image, index) => {
    ...
    const cardWidth = 150; // 固定卡片宽度
    const cardHeight = Math.max(100, cardWidth / aspectRatio); // 计算卡片高度
...
function updateSelectedCount() {
  ...
  const imageCountElement = document.getElementById('imageCount');
  imageCountElement.textContent = `${totalCount}  (${selectedCount} )`;
}
...
function batchDownload(downloadType = 'individual') {
  ...
  // 获取当前标签页标题作为文件夹名
  chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
    const folderName = tabs[0] ? tabs[0].title.replace(/[^a-zA-Z0-9\u4e00-\u9fa5]/g, '_') : 'PicSee';
